...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non WCC unit input in lnReqAmount, we generate the URI with WCC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::WCC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
