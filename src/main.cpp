...
// Copyright (c) 2013 The worldcupcoin developers
...
uint256 hashGenesisBlock("0x00a6bb5a5ffe883d7760450dc73087ab159dd5205af5a27d6cb9cb2454e66116");
static const unsigned int timeGenesisBlock = 1400662742;
...
const string strMessageMagic = "worldcupcoin Signed Message:\n";
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x0076f15adb1f7dcecba0fd17c34898904b041bac7a4d4af073b0afe42d020e30");
    }
...
*/
        ...
        // Genesis block
        const char* pszTimestamp = "Bitcoin Blockchain 21/May/2014 Hight:301860 Hash:00...afbd757ec17c1950fd8e93f6";
		...
		CBlock blockGenesis;
        blockGenesis.vtx.push_back(txNew);
        blockGenesis.hashPrevBlock = 0;
        blockGenesis.hashMerkleRoot = blockGenesis.BuildMerkleTree();
        blockGenesis.nVersion = 1;
        blockGenesis.nTime    = timeGenesisBlock;
        blockGenesis.nBits    = bnProofOfWorkLimit.GetCompact();
        blockGenesis.nNonce   = 12059739;

        if (fTestNet)
        {
            blockGenesis.nTime    = 1400655881;//ALTCOIN-WCC
            blockGenesis.nNonce   = 905553423;
        }

        //// debug print
		uint256 hashGenesisNew = blockGenesis.GetHash();

		//fine nonce
        while (hashGenesisNew > bnProofOfWorkLimit.getuint256()){
            if (++blockGenesis.nNonce==0) break;
            hashGenesisNew = blockGenesis.GetHash();
		...
		if(hashGenesisNew <= bnProofOfWorkLimit.getuint256())
		{
			...
			printf("hashGenesisBlock= 0x%s\n", hashGenesisNew.ToString().c_str());
            //printf("%s\n", hashGenesisBlock.ToString().c_str());
            printf("hashMerkelRoot= 0x%s\n", blockGenesis.hashMerkleRoot.ToString().c_str());
			printf("nNonce= %u\n",blockGenesis.nNonce);
       ...
       // printf("%s\n", blockGenesis.hashMerkleRoot.ToString().c_str());
        blockGenesis.print();
        assert(blockGenesis.hashMerkleRoot == uint256("0xf6f776db58930e0b22f9ed07122a458661960ea4da0ae4fcb204e4ba43fbdec6"));
        assert(hashGenesisNew == hashGenesisBlock);

       
        // Start new block file
        try {
            unsigned int nBlockSize = ::GetSerializeSize(blockGenesis, SER_DISK, CLIENT_VERSION);
            CDiskBlockPos blockPos;
            CValidationState state;
            if (!FindBlockPos(state, blockPos, nBlockSize+8, 0, blockGenesis.nTime))
                return error("LoadBlockIndex() : FindBlockPos failed");
            if (!blockGenesis.WriteToDisk(blockPos))
                return error("LoadBlockIndex() : writing genesis block to disk failed");
            if (!blockGenesis.AddToBlockIndex(state, blockPos))
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("worldcupcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("worldcupcoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("worldcupcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("worldcupcoin-miner");
    ...
    try { loop {
        ...
        printf("Running worldcupcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
//            unsigned int nNonceFound;
    ...
    {
        printf("worldcupcoinMiner terminated\n");
        throw;
    }
}
